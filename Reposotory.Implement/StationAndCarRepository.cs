...
namespace Reposotory.Implement
{
    ...
    public class StationAndCarRepository
    {
        ...
        public List<MotorRentObj> GetAllMotorRent()
        {
            ...
            SQL += " AND CarNo NOT IN (SELECT CarNo FROM TB_OrderMain M WITH(NOLOCK) WHERE car_mgt_status < 16 AND cancel_status = 0 AND booking_status<5 AND ProjType=4 AND start_time > DATEADD(MONTH,-1,dbo.GET_TWDATE()) ) ";
            SqlParameter[] para = new SqlParameter[2];
            string term = "";
        ...
        public List<ProjectAndCarTypeDataForMotor> GetProjectOfMotorRent(string CarNo, DateTime SDate, DateTime EDate)
        {
            ...
            INNER JOIN TB_iRentStation irs ON irs.StationID = VW.StationID AND irs.IsNormalStation=4

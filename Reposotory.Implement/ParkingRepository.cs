...
namespace Reposotory.Implement
{
    ...
    public class ParkingRepository : BaseRepository
    {
        ...
        /// <param name="ParkingName"></param>
        /// <returns></returns>
        public List<BE_ParkingData_Moto> GetTransParking_Moto(string ParkingName)
        {
            bool flag = false;
            List<ErrorInfo> lstError = new List<ErrorInfo>();
            List<BE_ParkingData_Moto> lstParking = null;
            int nowCount = 0;
            string SQL = "SELECT  [ParkingID],[ParkingName],[ParkingCNT],[ParkingAddress],[ParkingLng] ,[ParkingLat],[OpenTime],[CloseTime],[ParkingMark] FROM [dbo].[TB_ParkingData_Motor] ";
            SqlParameter[] para = new SqlParameter[2];
            string term = "";
            if (string.IsNullOrEmpty(ParkingName) == false && string.IsNullOrWhiteSpace(ParkingName) == false)
            {
            ...
            SQL += " ORDER BY ParkingID ASC ";
            lstParking = GetObjList<BE_ParkingData_Moto>(ref flag, ref lstError, SQL, para, term);
            return lstParking;
        }

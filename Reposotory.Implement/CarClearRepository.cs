   ...
   public  class CarClearRepository:BaseRepository
    {
        ...
        public List<BE_CleanDataWithoutPIC> GetCleanDataWithOutPic(string SDate, string EDate, string carid, string objStation, string userID, int status, ref List<ErrorInfo> lstError)
        {
            ...
            string SQL = "SELECT  distinct Account,OrderNum,UserID,outsideClean,insideClean,rescue,dispatch,Anydispatch,Maintenance,OrderStatus,remark,BookingStart,BookingEnd,CarNo,lastCleanTime,lastRentTimes,lend_place FROM VW_BE_CleanDataQueryWithOutPIC ";
            SqlParameter[] para = new SqlParameter[8];
            string term = "";
            int nowCount = 0;
            DateTime SD, ED;
            ...
            if ("" != term)
            {
                SQL += " WITH(NOLOCK) WHERE (" + term + ")  GROUP BY Account,OrderNum,UserID,outsideClean,insideClean,rescue,dispatch,Anydispatch,Maintenance,OrderStatus,remark,BookingStart,BookingEnd,CarNo,lastCleanTime,lastRentTimes,lend_place";
            }

...
BEGIN
   ...
   begin       
	  ...
	  set @re = dbo.FN_MotoRentPara(@xMins,@Price,@PriceH,@MaxPrice,@MaxPriceH,@strSD)
   end
   else if @xdays = 1 --隔天
   begin
	  ...
	  set @re += dbo.FN_MotoRentPara(@day_one_mins, @Price, @PriceH, @MaxPrice, @MaxPriceH, @strSD) --首日花費
	  set @re += dbo.FN_MotoRentPara(@day_end_mins, @Price, @PriceH, @MaxPrice, @MaxPriceH, @strED) --尾日花費
   end
   else if @xdays > 1 
   begin
       ...
       declare @tm_sd_end datetime = convert(datetime, (convert(varchar, dateadd(day,1, @SD) , 111)+ ' ' + '00:00:00')) --首日結束
	    ...
	    set @re += dbo.FN_MotoRentPara(@day_one_m, @Price, @PriceH, @MaxPrice, @MaxPriceH, @strSD) --首日花費

	    declare @day_end_m int = DATEDIFF(minute,@tm_ed_star,@ED)
	    set @re += dbo.FN_MotoRentPara(@day_end_m, @Price, @PriceH, @MaxPrice, @MaxPriceH, @strED) --尾日花費 
		...
		begin				
			...
			set @re += dbo.FN_MotoRentPara(24*60, @Price, @PriceH, @MaxPrice, @MaxPriceH, @str_date)

...
BEGIN
   declare @re float = 0
   if @OrderPrice > 0 and @BookingSD <= @BookingED and @FinalSD <= @FinalED 
   begin      
	   ...
	   begin		       		  
		   declare @unUseDays float = @ck_days
...
select @bok_mins[bok_mins], @bok_days[bok_days], @bok_xmins[bok_xmins]
select @ck_mins[ck_mins], @ck_days[ck_days], @ck_xmins[ck_xmins]
*/
GO
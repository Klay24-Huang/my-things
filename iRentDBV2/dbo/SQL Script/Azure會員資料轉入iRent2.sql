...
DELETE FROM TB_MemberData_TransTest
...
SELECT
	...
	MEMPWD=ISNULL(A.MEMPWD,''),  --轉檔有NULL的問題
	...
	Audit=CASE WHEN ISNULL(D.AUDITRESULT,'')='' THEN 0 WHEN ISNULL(D.AUDITRESULT,'')='Y' THEN 1 WHEN ISNULL(D.AUDITRESULT,'')='N' THEN 3 ELSE 0 END,		--是否通過審核(0:未審;1:已審;2:審核不通過)
	...
	RentType=CASE WHEN A.IRENTFLG='N' OR A.MEMGROUP<>'Y' THEN 0		--可租車類別：0:無法;1:汽車;2:機車;3:全部
				...
				ELSE 0 END
FROM MEMBER_NEW_AZTEST A WITH(NOLOCK)
LEFT JOIN MEMBER_API B WITH(NOLOCK) ON B.MEMRFNBR=A.MEMRFNBR

...
BEGIN TRY
	...
	BEGIN
		...
		ELSE
		BEGIN
			SELECT @CID=ISNULL([CID],''),
				@deviceToken=ISNULL([deviceToken],''),
				@IsCens=ISNULL([IsCens],''),
				@IDNO=ISNULL(OrderMain.IDNO,'')
			FROM [TB_CarInfo] AS CarInfo WITH(NOLOCK)
			INNER JOIN TB_OrderMain AS OrderMain WITH(NOLOCK) ON OrderMain.CarNo=CarInfo.CarNo AND OrderMain.order_number=@OrderNo
			INNER JOIN TB_MemberData AS MemberData WITH(NOLOCK) ON MemberData.MEMIDNO=OrderMain.IDNO

...
**			 |			  |
*****************************************************************/
CREATE PROCEDURE [dbo].[usp_INSTmpFeedBackPIC]
	@IDNO                   VARCHAR(10)           ,
	@OrderNo                BIGINT                ,
	@Token                  VARCHAR(1024)         ,
	@SEQNO                  TINYINT               ,
	@FeedBackFile           VARCHAR(MAX)          ,
	@LogID                  BIGINT                ,
...
SET @FunName='usp_INSTmpFeedBackPIC';
SET @IsSystem=0;
SET @ErrorType=0;
SET @IsSystem=0;
SET @hasData=0;
...
SET @Token    =ISNULL (@Token    ,'');
		...
		BEGIN TRY
		 ...
		 IF @Error=0
		 BEGIN
			SELECT @hasData=COUNT(1) FROM TB_tmpFeedBackPIC WHERE OrderNo=@OrderNo AND SEQNO=@SEQNO ;
			IF @hasData=0
			BEGIN
			...
			END
		 END
		 SELECT SEQNO,1 AS HasUpload FROM TB_tmpFeedBackPIC WHERE OrderNo=@OrderNo;
		--寫入錯誤訊息
		    IF @Error=1
			BEGIN
...
GO
EXECUTE sp_addextendedproperty @name = N'Owner', @value = N'Eric', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_INSTmpFeedBackPIC';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'上傳取車回饋照片', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_INSTmpFeedBackPIC';


GO
EXECUTE sp_addextendedproperty @name = N'IsActive', @value = N'1:使用', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_INSTmpFeedBackPIC';


GO
EXECUTE sp_addextendedproperty @name = N'Comments', @value = N'', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_INSTmpFeedBackPIC';
...
SET @DeadLine=GETDATE();
		...
		BEGIN TRY
		 ...
		 BEGIN
		    ...
		    IF @Mode=0
			BEGIN
				SELECT TOP 1 @VerifyCodeID=ISNULL(VerifyCodeID,0),@tmpVerifyCode=ISNULL(VerifyNum,''),@DeadLine=ISNULL(DeadLine,GETDATE()) FROM TB_VerifyCode WHERE IDNO=@IDNO AND Mode=0 AND IsVerify=0 ORDER BY SendTime DESC; 
			...
			ELSE IF @Mode=1
			BEGIN
				SELECT TOP 1 @VerifyCodeID=ISNULL(VerifyCodeID,0),@tmpVerifyCode=ISNULL(VerifyNum,''),@DeadLine=ISNULL(DeadLine,GETDATE()) FROM TB_VerifyCode WHERE IDNO=@IDNO AND Mode=1  ORDER BY SendTime DESC; 
				IF @VerifyCodeID>0 AND @tmpVerifyCode=@VerifyCode 
				BEGIN
					IF @NowTime<@DeadLine
					BEGIN
			...
			BEGIN
				...
				SELECT TOP 1 @VerifyCodeID=ISNULL(VerifyCodeID,0),@tmpVerifyCode=ISNULL(VerifyNum,''),@DeadLine=ISNULL(DeadLine,GETDATE()) FROM TB_VerifyCode WHERE IDNO=@IDNO AND Mode=2 AND OrderNum=@OrderNum  ORDER BY SendTime DESC;
				IF @VerifyCodeID>0 AND @tmpVerifyCode=@VerifyCode
				BEGIN
				    IF @NowTime<@DeadLine
					BEGIN

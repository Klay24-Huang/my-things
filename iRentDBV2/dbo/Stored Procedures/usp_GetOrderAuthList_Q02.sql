...
* Example  : 
***********************************************************************************************/
CREATE PROCEDURE [dbo].[usp_GetOrderAuthList_Q02]
	@GateNo                INT,      
	@Retry                 INT,
...
SET @FunName='usp_GetOrderAuthList_Q02';
SET @IsSystem=0;
SET @ErrorType=0;
SET @IsSystem=0;
SET @hasData=0;
...
BEGIN TRY
	SELECT TOP 20
		...
		A.AuthFlg
	INTO #TB_OrderAuth
	FROM TB_OrderAuth A WITH(NOLOCK) 
	...
	SET U_SYSDT=CASE WHEN A.AuthFlg=9 THEN U_SYSDT ELSE @NowTime END,
		...
		U_PRGID='GetOrderAuthList',
		U_USERID='GetOrderAuthList'
	FROM TB_OrderAuth A
...
EXECUTE sp_addextendedproperty @name = N'Platform', @value = N'API', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'PROCEDURE', @level1name = N'usp_GetOrderAuthList_Q02';
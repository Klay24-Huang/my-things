...
SET @tmpOrderNum=0;
		...
		BEGIN TRY
		 ...
		 BEGIN
			  ...
			  SET @hasData=0;
			   SELECT @tmpOrderNum=ISNULL(order_number,0),@CarNo=ISNULL(OrderMain.CarNo,''),@SD=ISNULL(start_time,'1911-01-01 00:00:00'),@ED=ISNULL(stop_time,'1911-01-01 00:00:00'),@StationID=ISNULL(lend_place,''),@IDNO=ISNULL(IDNO,''),@ProjID=ISNULL(OrderMain.ProjID,''),@CarType=ISNULL(VW.CarTypeGroupCode,'')
			   FROM TB_OrderMain AS OrderMain WITH (NOLOCK)
			   INNER JOIN TB_CarInfo AS CarInfo ON CarInfo.CarNo=OrderMain.CarNo
			   INNER JOIn  [dbo].[VW_GetFullProjectCollectionOfCarTypeGroup] AS VW ON VW.PROJID=OrderMain.ProjID AND VW.CARTYPE=CarInfo.CarType
			  ...
			  BEGIN
				...
				--判斷有沒有預約到車
				 ...
				 BEGIN
						...
						UPDATE TB_CarInfo SET RentCount=RentCount+1 WHERE CarNo=@NewCarNo;
						UPDATE TB_CarInfo SET RentCount=RentCount-1 WHERE CarNo=@CarNo AND (RentCount-1)>0 ;
						 --寫入歷史記錄

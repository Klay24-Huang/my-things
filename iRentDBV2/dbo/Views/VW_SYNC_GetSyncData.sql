CREATE VIEW [dbo].[VW_SYNC_GetSyncData]
	AS 
SELECT ISNULL(BookingMain.order_number, 0) AS OrderNum,
       BookingMain.IDNO,
       BookingMain.CarNo,
       LoginData.MEMCNAME + CASE substring(LoginData.MEMIDNO, 2, 1) WHEN '1' THEN N' 先生' WHEN '2' THEN N' 女士' ELSE '' END AS Title,
	   BookingMain.start_time AS StartTime,
	   BookingMain.stop_time AS StopTime,
	   ISNULL(BookingMain.fine_Time, N'1911-01-01 00:00:00') AS FineTime,
	   ISNULL(BookingDetail.final_start_time, N'1911-01-01 00:00:00') AS FinalStartTime,
	   ISNULL(BookingDetail.final_stop_time, N'1911-01-01 00:00:00') AS FinalStopTime,
	   ISNULL(LoginData.CARDNO, N'') AS CardNo,
	   BookingMain.car_mgt_status,
	   BookingMain.booking_status,
	   BookingMain.cancel_status,
	   CarInfo.CID,
	   ISNULL(LoginData.PushREGID, '') AS DeviceToken ,
	   BookingMain.ProjID,
	   ISNULL(Project.PROJTYPE, - 1) AS PROJTYPE,
	   BookingMain.spec_status,
	   BookingMain.stop_pick_time AS stop_pick_time

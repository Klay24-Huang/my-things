CREATE VIEW [dbo].[VW_BE_GetCarDashBoardData]
  ...
  , ISNULL(Station.[Location],'') AS StationName,ISNULL(Car.CarNo,'未綁定車輛') AS CarNo,ISNULL(Car.available,1) AS NowStatus,ISNULL(deviceType,0) AS deviceType
  ,ISNULL(CarInfo.deviceToken,'') AS deviceToken,CarStatus.CID,ISNULL(CarTypeName,'') AS CarTypeName

    ...
    public class MixAuthModeController : ApiController
    {
        ...
        public Dictionary<string, object> DoMixAuthMode(Dictionary<string, object> value)
        {
            ...
            if (flag)
            {
                ...
                if (apiInput.CreditAmount < 0 || apiInput.WalletAmount<0)
                {
                    flag = false;
                    errCode = "ERR900";
                }
            ...
            if (flag && isGuest == false)
            {
                ...
                if (apiInput.PayType == 0)
                {
                    ...
                    if (flag)
                    {
                        if (Amount > (apiInput.WalletAmount + apiInput.CreditAmount))
                        {
                            flag = false;
                            errCode = "ERR211";
                        }
                            ...
                            if (hasFind)//有找到，可以做扣款
                            {
                                ...
                                if (Amount > 0)
                                {
                                    Domain.WebAPI.Input.Taishin.AuthItem item = new Domain.WebAPI.Input.Taishin.AuthItem()
                                    {
                                        ...
                                        NonPoint = "N",
                                        NonRedeem = "N",
                                        Price = apiInput.CreditAmount.ToString() + "00",
                                        Quantity = "1"
                                    };
                                    PartOfCreditCardAuth WSAuthInput = new PartOfCreditCardAuth()
                                    {
                                        ...
                                        RequestParams = new Domain.WebAPI.Input.Taishin.AuthRequestParams()
                                        {
                                            ...
                                            NonRedeemAmt = apiInput.CreditAmount.ToString() + "00",
                                            NonRedeemdescCode = "",
                                            Remark1 = "",
                                            Remark2 = "",
                                            Remark3 = "",
                                            ResultUrl = BindResultURL,
                                            TradeAmount = apiInput.CreditAmount.ToString() + "00",
                                            TradeType = "1",
                                            UseRedeem = "N"

...
{
    public class GetMotorParkingDataController : ApiController
    {
        private string connetStr = ConfigurationManager.ConnectionStrings["IRent"].ConnectionString;
        [HttpPost]
        [ActionName("GetMotorParkingData")]
        public Dictionary<string, object> DoGetMotorParkingData(Dictionary<string, object> value)
        {
            #region 初始宣告
            ...
            string funName = "GetMotorParkingDataController";
            Int64 LogID = 0;
            Int16 ErrType = 0;
            ...
            if (flag)
            {
                ...
                {
                    parkings = _repository.GetAllMotorParking();
                }
                else if (apiInput.ShowALL == 0 && apiInput.Mode.Value == 2) //方圓顯示全部
                {
                    parkings = _repository.GetAllMotorParking(apiInput.Latitude.Value, apiInput.Longitude.Value, apiInput.Radius.Value);
                }
                else if (apiInput.ShowALL == 1 && apiInput.Mode.Value < 2)
                {
                    parkings = _repository.GetAllMotorParkingByType(apiInput.Mode.Value);
                }
                else
                {
                    parkings = _repository.GetAllMotorParkingByType(apiInput.Mode.Value, apiInput.Latitude.Value, apiInput.Longitude.Value, apiInput.Radius.Value);
                }

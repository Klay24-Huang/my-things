...
namespace WebAPI.Controllers
{
    ...
    /// 使用信用卡付款
    /// </summary>
    public class CreditAuthReturnJobController : ApiController
    {
        ...
        public Dictionary<string, object> DoCreditAuthJob()
        {
            ...
            string funName = "CreditAuthReturnJobController";
            Int64 LogID = 0;
            Int16 ErrType = 0;
            ...
            Int64 tmpOrder = 0;
            int Amount = 0;
            List<OrderAuthReturnList> OrderAuthList = null;
            ...
            if (flag)
            {
                ...
                string SPName = new ObjType().GetSPName(ObjType.SPType.GetOrderAuthReturnList);
                ...
                OrderAuthList = new List<OrderAuthReturnList>();
                ...
                if (flag)
                {
                    if (OrderAuthList.Count == 0)
                    {
                        flag = false;
                        errCode = "ERR203";
                        logger.Trace("GetOrderAuthReturnList Error:"+JsonConvert.SerializeObject(lstError));
                    }
                }
            }
            #endregion
            if (flag)
            {
                logger.Trace("OrderAuthReturnList Count:" + OrderAuthList.Count.ToString());
                for (int i = 0; i < OrderAuthList.Count; i++)
                {
                    ...
                    if (OrderAuthList[i].returnAmt > 0)       //有錢才刷
                    {
                    ...
                    SPInput_UpdateOrderAuthList UpdateOrderAuthList = new SPInput_UpdateOrderAuthList()
                    {
                        ...
                        AuthFlg = WSRefundOutput.ResponseParams.ResultCode != "1000" ? -1 : 1,
                        AuthCode = WSRefundOutput.ResponseParams.ResultCode,
                        AuthMessage = WSRefundOutput.ResponseParams.ResultMessage,
                        OrderNo = OrderAuthList[i].order_number,
                        transaction_no = WSRefundOutput.OriRequestParams.RequestParams.MerchantTradeNo,
                        Reward = 0
                    };
                    //20201228 ADD BY ADAM REASON.因為目前授權太久會有回上一頁重新計算的問題
                    //                            所以把存檔功能先提早完成再進行信用卡授權
                    string SPName = new ObjType().GetSPName(ObjType.SPType.UpdateOrderAuthReturnList);
                    ...
                    if (flag == false)
                    {
                        logger.Trace("UpdateOrderAuthReturnList Params:" + JsonConvert.SerializeObject(UpdateOrderAuthList));
                        logger.Trace("UpdateOrderAuthReturnList Error:" + JsonConvert.SerializeObject(lstError));
                    }

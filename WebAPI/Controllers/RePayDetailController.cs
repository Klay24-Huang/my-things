...
namespace WebAPI.Controllers
{
    ...
    public class RePayDetailController : ApiController
    {
        ...
        public Dictionary<string, object> DoRePayDetail(Dictionary<string, object> value)
        {
            ...
            IAPI_RePayDetail apiInput = null;
            OAPI_GetPayDetail apiOutput = new OAPI_GetPayDetail();
            #endregion
            ...
            try
            {
                ...
                if (flag)
                {
                    apiInput = JsonConvert.DeserializeObject<IAPI_RePayDetail>(Contentjson);
                ...
                if (flag)
                {
                    ...
                    if (flag)
                    {
                        ...
                        apiOutput.MonBase = new List<MonBase>();
                        apiOutput.CanUseDiscount = 1;   //先暫時寫死，之後改專案設定，由專案設定引入
                        apiOutput.CanUseMonthRent = 1;  //先暫時寫死，之後改專案設定，由專案設定引入
                        apiOutput.CarRent = new CarRentBase();
                        apiOutput.DiscountAlertMsg = "";
                        apiOutput.IsMonthRent = 0;  //先暫時寫死，之後改專案設定，由專案設定引入，第二包才會引入月租專案
                        apiOutput.IsMotor = (ProjType == 4) ? 1 : 0;    //是否為機車
                        apiOutput.MonthRent = new MonthRentBase();  //月租資訊
                        apiOutput.MotorRent = new MotorRentBase();  //機車資訊
                        apiOutput.PayMode = (ProjType == 4) ? 1 : 0;    //目前只有機車才會有以分計費模式
                        apiOutput.ProType = ProjType;
                        apiOutput.Rent = new RentBase() //訂單基本資訊
                        {
                        ...
                        if (ProjType == 4)
                        {
                            ...
                            apiOutput.MotorRent = new MotorRentBase()
                            {
                    ...
                    if (flag && ProjType != 4) //20201224 add by adam 問題未確定前先關掉車麻吉
                    {
                        ...
                        try
                        {
                            ...
                            if (magi_Re != null)
                            {
                                ...
                                //flag = magi_Re.flag;
                                apiOutput.Rent.ParkingFee = magi_Re.ParkingFee;
                            }
                        }
                        catch (Exception ex)
                        {
                    ...
                    if (flag && ProjType != 4)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            {
                                apiOutput.Rent.ParkingFee += CityParkingPrice;
                            }
                        }
                    ...
                    if (flag)
                    {
                        ...
                        if (mon_re != null)
                        {
                            ...
                            apiOutput.IsMonthRent = mon_re.IsMonthRent;
                            if (UseMonthMode)
                            {
                                ...
                                if (ProjType == 4)
                                    apiOutput.Rent.CarRental = mon_re.CarRental;//機車用
                                else
                    ...
                    if (flag)
                    {
                        ...
                        if (ProjType == 4)
                        {
                            ...
                            {
                                apiOutput.MonthRent.HoildayRate = monthlyRentDatas[0].HoildayRateForMoto;
                                apiOutput.MonthRent.WorkdayRate = monthlyRentDatas[0].WorkDayRateForMoto;
                            }
                            else
                            {
                                ...
                                {
                                    carInfo = xre;
                                    apiOutput.Rent.CarRental = xre.RentInPay;
                            ...
                            if (item.Insurance == 1)    // 有安心服務
                            {
                                ...
                                apiOutput.Rent.InsurancePurePrice = MainInsurancePrice + (item.JointPassenger * JointInsurancePrice);
                            }
                            #endregion
                            apiOutput.Rent.RentBasicPrice = item.BaseMinutesPrice;
                        }
                        else
                        {
                            if (UseMonthMode)   //true:有月租;false:無月租
                            {
                                apiOutput.Rent.CarRental = CarRentPrice;
                                apiOutput.MonthRent.HoildayRate = monthlyRentDatas == null ? 0 : monthlyRentDatas[0].HoildayRateForCar;
                                apiOutput.MonthRent.WorkdayRate = monthlyRentDatas == null ? 0 : monthlyRentDatas[0].WorkDayRateForCar;
                            }
                            else
                            {
                                CarRentPrice = car_inPrice;//未逾時租用費用
                                if (hasFine)
                                    apiOutput.Rent.OvertimeRental = car_outPrice;//逾時費用
                            ...
                            if (item.Insurance == 1)    // 有安心服務
                            {
                                ...
                                apiOutput.Rent.InsurancePurePrice = MainInsurancePrice + (item.JointPassenger * JointInsurancePrice);
                                apiOutput.Rent.InsuranceExtPrice = MainyInsuranceFinePrice + (item.JointPassenger * JointInsuranceFinePrice);
                            ...
                            apiOutput.CarRent.MilUnit = MillageUnit;
                            ...
                            {
                                apiOutput.Rent.MileageRent = 0;
                            }
                            else
                            {
                                apiOutput.Rent.MileageRent = Convert.ToInt32(MillageUnit * (item.end_mile - item.start_mile));
                            }
                            trace.FlowList.Add("里程費計算");
                            #endregion
                        }

                        apiOutput.Rent.OvertimeRental = car_outPrice;
                        apiOutput.Rent.ActualRedeemableTimeInterval = ActualRedeemableTimePoint.ToString();
                        apiOutput.Rent.RemainRentalTimeInterval = (TotalRentMinutes > 0 ? TotalRentMinutes : 0).ToString();
                        apiOutput.Rent.TransferPrice = (item.init_TransDiscount > 0) ? item.init_TransDiscount : 0;
                        //20201202 ADD BY ADAM REASON.ETAG費用
                        apiOutput.Rent.ETAGRental = etagPrice;

                        #region 轉乘優惠只能抵租金
                        int xCarRental = apiOutput.Rent.CarRental;
                        int xTransferPrice = apiOutput.Rent.TransferPrice;
                        int FinalTransferPrice = (xCarRental - xTransferPrice) > 0 ? xTransferPrice : xCarRental;
                        apiOutput.Rent.TransferPrice = FinalTransferPrice;
                        xCarRental = (xCarRental - FinalTransferPrice);
                        #endregion

                        #region 總價計算
                        // 總價 = 車輛租金 + 停車費 + 里程費 + 逾時費用 + 安心服務費用 + 安心服務費用(逾時) + ETAG費用
                        var xTotalRental = xCarRental + apiOutput.Rent.ParkingFee + apiOutput.Rent.MileageRent + apiOutput.Rent.OvertimeRental + apiOutput.Rent.InsurancePurePrice + apiOutput.Rent.InsuranceExtPrice + apiOutput.Rent.ETAGRental;

                        #region 2021春節訂金已不使用-2022春節已開啟
                        // 20211026 UPD BY YEH REASON:2021春節訂金已不使用
                        //xTotalRental -= UseOrderPrice;  //預繳定金扣抵

                        //apiOutput.UseOrderPrice = UseOrderPrice;
                        //20220126 ADD BY ADAM REASON.春節訂金改用預授權金額帶入
                        apiOutput.UseOrderPrice = PreAmount;
                        //apiOutput.FineOrderPrice = OrderPrice - UseOrderPrice;//沒收訂金
                        apiOutput.FineOrderPrice = UseOrderPrice;//改罰金
                        ...
                        if (UseOrderPrice > 0)
                        {
                            //apiOutput.ReturnOrderPrice = (-1) * xTotalRental;
                            int orderNo = Convert.ToInt32(item.OrderNo);
                            //carRepo.UpdNYPayList(orderNo, apiOutput.ReturnOrderPrice);
                            carRepo.UpdNYPayList(orderNo, UseOrderPrice);

                            //不含退還訂金
                            //OrderPrice = OrderPrice - apiOutput.ReturnOrderPrice;
                            //OrderPrice = OrderPrice > 0 ? OrderPrice : 0;
                            //apiOutput.UseOrderPrice = OrderPrice;
                        }
                        #endregion
                        ...
                        apiOutput.Rent.TotalRental = xTotalRental;

                        // 20211229 UPD BY YEH REASON:output增加預授權金額、差額
                        apiOutput.Rent.PreAmount = PreAmount;
                        apiOutput.Rent.DiffAmount = DiffAmount;
                        ...
                        if (ProjType == 4)  // 機車
                        {
                            apiOutput.Rent.RentalTimeInterval = carInfo.RentInMins.ToString();  //租用時數(未逾時)
                            apiOutput.Rent.CanDiscountTime = carInfo.DiscRentInMins;
                            apiOutput.Rent.ActualRedeemableTimeInterval = carInfo.AfterDiscRentInMins.ToString();
                            apiOutput.Rent.UseMonthlyTimeInterval = carInfo.useMonthDisc.ToString();
                            apiOutput.Rent.UseNorTimeInterval = carInfo.useDisc.ToString();
                            apiOutput.Rent.UseGiveMinute = carInfo.UseGiveMinute;
                            apiOutput.Rent.RemainRentalTimeInterval = carInfo.AfterDiscRentInMins.ToString();//未逾時折扣後的租用時數
                        ...
                        else
                        {
                            ...
                            {
                                apiOutput.Rent.RentalTimeInterval = car_payInMins.ToString(); //租用時數(未逾時)
                                apiOutput.Rent.CanDiscountTime = CanDiscountTime;
                                apiOutput.Rent.ActualRedeemableTimeInterval = (car_payInMins - UseGiveMinute).ToString(); //可折抵租用時數
                                apiOutput.Rent.UseNorTimeInterval = Discount.ToString();
                                apiOutput.Rent.UseGiveMinute = UseGiveMinute;
                                apiOutput.Rent.RemainRentalTimeInterval = (car_payInMins - Discount - UseGiveMinute).ToString();    //未逾時折抵後的租用時數
                        ...
                        #region SP存檔
                        string SPName = "usp_CalFinalPrice_Re";
                        SPInput_CalFinalPrice SPInput = new SPInput_CalFinalPrice()
                        {
                            IDNO = IDNO,
                            OrderNo = tmpOrder,
                            final_price = apiOutput.Rent.TotalRental,
                            pure_price = apiOutput.Rent.CarRental,
                            mileage_price = apiOutput.Rent.MileageRent,
                            Insurance_price = apiOutput.Rent.InsurancePurePrice + apiOutput.Rent.InsuranceExtPrice,
                            fine_price = apiOutput.Rent.OvertimeRental,
                            ...
                            Etag = apiOutput.Rent.ETAGRental,
                            parkingFee = apiOutput.Rent.ParkingFee,
                            TransDiscount = apiOutput.Rent.TransferPrice,
                            Token = Access_Token,
                        ...
                        trace.traceAdd(nameof(apiOutput), apiOutput);
